...
## 线程池
### std::function
std::function<>模板是 C++11 引入的灵活的可调用对象包装器，可以存储任意类型的可调用对象（普通函数、lambda、仿函数等）。常用于需要将可调用对象作为参数传递、保存或延迟执行的场景。
```cpp
#include <functional>
#include <iostream>
...
int main() {
    ...
    func();  // 调用 std::function 包装的函数
    return 0;
}
```
线程池的实现中`std::queue<std::function(void())> tasks;`定义了一个queue容器，利用vector的模板机制存储经过std::function包装过的void函数等可调用对象。
...
```
没有模板的情况下，函数参数& f为左值引用，使用左值例如`int a`,&& f为右值引用，引用右值，如`8`；
模板中的&&是万能引用，不是右值引用；他既可以接受左值又可以接受右值！他会根据接受的参数确定时左值还是右值！但是右值引用本身是左值。万能引用解决了右值引用不能饮用左值的问题。
完美转发std::forward实现了完美转发，保留对象的原生属性。std::forward<F>(f)保留了f的原生属性，传入左值或右值都不会修改自身属性。

### emplace_back
`vector.emplace_back(args...);`
args... 是要传递给对象构造函数的参数。
emplace_back 直接在容器的内存中构造对象。

emplace_back 是 C++11 引入的 std::vector（以及其他标准库容器）的方法。它的作用是直接在容器的末尾构造对象，相比于 push_back，emplace_back 更高效，因为它避免了不必要的临时对象创建和移动/拷贝。
```cpp
...
```
这里由于线程std::thread的构造函数至少需要一个回调函数，emplace_back的参数使用lamba表达式[this] { this->worker(); }将worker的构造函数作为回调函数。

### 移动语义
所谓移动语义，就是将一个左值的资源转换为右值移动到另一个左值；避免拷贝；结果就是将原来指针A指向的内存交给了指针B，指针A不指向任何内存。相当于B偷走了A的东西。
`task = std::move(tasks.front());`通过move将左值引用转化为右值引用；
队列中的任务(tasks.front())的资源会被转移到 task 中，而队列中的元素则可能变为空（或被移为无效状态，取决于 std::function 的实现）。
但是并不会自动从队列中删除该元素。